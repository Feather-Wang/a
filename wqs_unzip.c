...
int main(int argc, const char *argv[])
{
    ...
    */
    ...
    printf("CCL=\n");
    for(index_i = 0; index_i < (bits_header.hclen+4); index_i++)
    {
        if( bit_flag < 8 )
        {
    ...
    printf("CL1--------------------------------------------------------------------\n");
    cl1_list = malloc(sizeof(array_unit_t)*(bits_header.hlit+257));
    if( NULL == cl1_list )
    {
        fprintf(stderr, "cl1_list malloc error, errno=[%d]\n", errno);
        return -1;
    }
    memset(cl1_list, 0x00, sizeof(array_unit_t)*(bits_header.hlit+257));
    cl_len_count = 0;
    cl1_list_num = 0;
    plies_max = 0;
    while( 1 )
    {
        ...
        */
        ...
        if( cl_len_count == (bits_header.hlit+257) )
        {
    ...
    printf("CL2--------------------------------------------------------------------\n");
    cl2_list = malloc(sizeof(array_unit_t)*(bits_header.hdist+1));
    if( NULL == cl2_list )
    {
        fprintf(stderr, "cl2_list malloc error, errno=[%d]\n", errno);
        return -1;
    }
    memset(cl2_list, 0x00, sizeof(array_unit_t)*(bits_header.hdist+1));
    cl_len_count = 0;
    cl2_list_num = 0;
    plies_max = 0;
    while( 1 )
    {
        ...
        */
        ...
        if( cl_len_count == (bits_header.hdist+1) )
        {
